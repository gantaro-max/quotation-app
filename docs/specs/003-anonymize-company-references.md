...
- **ステータス:** 完了（Claude Codeによる最終レビュー承認済み、2026-08-21）
...
Public化の最終精査で、コミットメッセージおよび匿名化コミット以前の全履歴（ファイル内容・ディレクトリパス）にも実社名が残存していることが判明した。当初の方針（本文中「Git履歴の書き換えは行わない」「スコープ外: Git履歴の書き換え」）を変更し、`git-filter-repo`により`main`・`feature/003-anonymize-company-references`両ブランチの全コミットを対象に、パスリネーム・テキスト置換・コミットメッセージ書き換えを実施した（force push実施済み）。以降、上記2箇所の記述は本追記により上書きされたものとする。
