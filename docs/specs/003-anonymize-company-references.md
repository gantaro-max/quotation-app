...
Public化の最終精査で、過去の履歴にも元企業を特定できる情報が残存していることが判明した。履歴の書き換えでは消し漏れのリスクが残るため、本リポジトリは匿名化後の最新状態のみを初回コミットとして新規作成した(2026-09-15)。あわせて、既定の営業所IDの固定値を `frontend/src/constants.ts` の定数に置き換え、利用者の職種名を一般的な表現に改めた。
