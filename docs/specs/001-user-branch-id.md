...
[docs/REQUIREMENTS.md](../REQUIREMENTS.md) 2章の通り、利用者は各拠点(営業所)に所属する営業担当者を想定しているが、`users` テーブルに所属営業所を保持するカラムが存在しない。結果、フロントエンドはログインユーザーの `branchId` を常に `undefined` として受け取り、[LoginPage.tsx](../../frontend/src/components/LoginPage.tsx) の既定営業所IDへのフォールバックにより、**誰がログインしても見積編集画面の営業所初期値が固定値になっている**。

## 要件

- ログインユーザーは1つの所属営業所 (branch_id) を持つ。
- ログインAPI (`POST /api/login`) のレスポンスに、そのユーザーの所属営業所IDを含める。
- フロントエンドは、返却された `branchId` を編集画面の営業所初期値として使用する。既存の既定営業所IDへのフォールバック(公開版では `frontend/src/constants.ts` の `DEFAULT_BRANCH_ID`)は、`branch_id` が未設定 (NULL) な異常系向けの安全策としてそのまま残してよい。
