...
| estimate_no          | VARCHAR(50)        |                        | 見積番号 (Q-yyyyMMdd...)    |
| version              | INT                | DEFAULT 1              | 版数                        |
| is_submitted         | TINYINT(1)         | DEFAULT 0              | 提出済フラグ                |
| created_by_user_id   | INT                | FK -> users(id)        | 作成者                      |
...
| total_amount         | DECIMAL(12, 0)     |                        | 税抜小計                    |
| **discount_amount**  | **DECIMAL(19, 2)** |                        | **値引き額** (NULL 許容)    |
| total_cost           | DECIMAL(12, 0)     |                        | 原価合計                    |
| total_profit         | DECIMAL(12, 0)     |                        | 粗利金額                    |
| profit_rate          | DECIMAL(5, 2)      |                        | 粗利率 (%)                  |
| grand_total          | DECIMAL(12, 0)     |                        | 見積合計金額 (税込)         |
| issue_date           | DATE               |                        | 発行日                      |
| remarks              | TEXT               |                        | 備考                        |
| attached_file_path   | VARCHAR(500)       |                        | 添付ファイルパス (OCR/仕入) |
| created_at           | DATETIME           | DEFAULT NOW            | 作成日時                    |
| updated_at           | DATETIME           | DEFAULT NOW            | 更新日時                    |
...
| row_type     | VARCHAR(20)    | 'normal'             | 行タイプ (normal, manufacturer 等) |
| item_code    | CHAR(9)        |                      | 商品コード                         |
| item_name    | VARCHAR(255)   |                      | 商品名・摘要                       |
| manufacturer | VARCHAR(100)   |                      | メーカー名                         |
| quantity     | DECIMAL(10, 2) |                      | 数量                               |
| cost_price   | DECIMAL(12, 0) |                      | 仕切単価 (原価)                    |
| unit_price   | DECIMAL(12, 0) |                      | 提出単価 (売価)                    |
